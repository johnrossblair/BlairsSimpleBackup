...
while true; do
    ...
    if [[ "$DAILY_LIMIT" =~ ^[0-9]+$ ]] && [ "$WEEKLY_LIMIT" -gt 0 ]; then
...
# Generate Script
OUTPUT_SCRIPT="$HOME/.test_script.sh"
...
WEEKLY_REMOVE=$WEEKLY_REMOVE
DAILY_REMOVE=$DAILY_REMOVE
...
if [ -e "\$WEEKLY_TIMESTAMP_FILE" ]; then
    ...
    if [ "\$WEEKLY_TIME_DIFF" -lt \$((7 * 86400)) ]; then
        ...
        echo "Running weekly full backup. Please do not close the terminal until complete."
    ...
    echo "First weekly backup. Running full backup. Please DO NOT close the terminal until complete."
    tar -cvpzf "\$FULL_BACKUP_DIR/full_backup_\$DATE.tar.gz" \\
        --exclude-from="\$EXCLUDE_LIST" \\
        --listed-incremental="\$SNAPSHOT_FILE" \\
        \$BACKUP_TARGET &> "\$FULL_BACKUP_DIR/full_backup_\$DATE.log"
...
if [ -e "\$DAILY_TIMESTAMP_FILE" ]; then
    ...
    echo "Running daily incremental backup. Please DO NOT close the terminal until complete."
    echo ""
...
#Ending Script
echo "Backup completed! Please check the log files to ensure everything was properly archived."
EOL
...
echo "Adding script to ~/.config/autostart"

mkdir -p "$HOME/.config/autostart"

AUTOSTART="$HOME/.config/autostart/test_script.sh.desktop"

cat <<EOL > "$AUTOSTART"
[Desktop Entry]
Type=Application
Exec=$HOME/.test_script.sh
